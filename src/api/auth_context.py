...
def get_current_context() -> RequestContext:
    ...
    if not ctx:
        if HOSTED_MODE:
            raise AuthenticationError("Unauthenticated request: no active request context established.")
...
class AuthContextResolver:
    ...
    def resolve_context(
        ...
        if user.status != "active":
            raise AuthorizationError(f"User account '{user_id}' is disabled or suspended.")

        # 3. Resolve Membership
        membership = self.repo.get_membership(company.company_id, user.user_id)
        if not membership:
            raise AuthorizationError(f"User '{user_id}' is not a member of company '{company_slug}'.")

        if membership.status != MembershipStatus.ACTIVE:
            raise AuthorizationError(f"Membership for user '{user_id}' in company '{company_slug}' is not active.")
