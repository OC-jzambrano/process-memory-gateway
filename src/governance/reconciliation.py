...
from src.models.enums import RunStatus, ExecutionEventType
...
def reconcile_run_by_operator(
    ...
    if not run:
        raise ReconciliationError(f"Execution run '{run_id}' not found for company '{company_id}'.")

    if run.status != RunStatus.RECONCILIATION_REQUIRED:
        raise ReconciliationError(f"Run '{run_id}' is in status '{run.status.value}', not 'reconciliation_required'.")
    ...
    if task_found:
        ...
        run.result_payload = {"task_name": task_found.name, "project_id": task_found.project_id}
        run.error_code = None
        ...
        failed_event = ExecutionEventRecord(
            ...
            details=sanitize_evidence({
                "operator": operator_user_id,
                "resolution": "confirmed_task_absent"
            }),
            created_at=now
        )
        run.status = RunStatus.FAILED
        run.error_code = "reconciliation_confirmed_not_created"
        run.error_detail = "Operator reconciliation verified task was not created in Odoo."
        repo.update_execution_run(run)
