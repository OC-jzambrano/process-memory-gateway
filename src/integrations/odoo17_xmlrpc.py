...
class TolerantHTMLParser(HTMLParser):
    def __init__(self):
        ...
        self._current_li: Optional[List[str]] = None
...
def normalize_html_text(raw_html: str) -> Tuple[str, List[str]]:
    ...
    cleaned_lis = [html.unescape(" ".join(item.split())) for item in parser.li_items if item.strip()]
    return html.unescape(plain_text), cleaned_lis

def verify_task_html(
    returned_html: str,
    expected_description: str,
    expected_dod: Optional[List[str]]
    ...
    if norm_expected_desc and norm_expected_desc not in plain_text:
        return False, f"Expected description text '{norm_expected_desc}' not found in task description."
    ...
    if expected_dod:
        ...
        if len(extracted_lis) < len(cleaned_expected_dod):
            return False, f"Expected {len(cleaned_expected_dod)} DoD items, found {len(extracted_lis)} in task HTML."
        ...
        for exp in cleaned_expected_dod:
            ...
            while curr < len(extracted_lis):
                ...
                curr += 1
            if not found:
                return False, f"Definition of Done item '{exp}' not found in expected order in task HTML."
...
class Odoo17XmlRpcExecutor(TaskExecutor):
    ...
    def __init__(
        ...
        self.default_project_id = int(default_project_id) if default_project_id is not None else None
    ...
    @classmethod
    def from_secret_dict(cls, secret_data: Dict[str, Any], default_project_id: Optional[int] = None) -> "Odoo17XmlRpcExecutor":
        ...
        proj_id = default_project_id or secret_data.get("default_project_id") or secret_data.get("DEFAULT_PROJECT_ID")
        if proj_id is None:
    ...
    def create_project_task_phase_aware(
        ...
        try:
            ...
            err_code = "access_denied" if ("AccessError" in fault_str or "Access Denied" in fault_str) else "create_rejected"
            return CreateTaskOutcome(
                phase=ExecutionPhase.CREATE_REJECTED,
                error_code=err_code,
                error_detail=fault_str
            )
        except (TimeoutError, socket.timeout, ConnectionResetError, ConnectionError, http.client.RemoteDisconnected, http.client.IncompleteRead) as e:
            return CreateTaskOutcome(
        ...
        try:
            ...
            logger.error("Read-back verification failed for task #%d: %s", task_id, str(e))
            return CreateTaskOutcome(
                phase=ExecutionPhase.VERIFICATION_FAILED,
                task_id=task_id,
                error_code="readback_exception",
                error_detail=f"Failed to read back created task #{task_id}: {str(e)}"
            )
        ...
        valid_content, err_msg = verify_task_html(desc_html, description, definition_of_done)
    ...
    def create_project_task(
        ...
        if outcome.error_code == "access_denied":
            raise OdooAccessDeniedError(outcome.error_detail or "Access denied to project.task.")
