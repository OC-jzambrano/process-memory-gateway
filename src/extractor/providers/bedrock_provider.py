...
class BedrockProvider(BaseLLMProvider):
    ...
    def extract(self, prompt: str, interaction_text: str) -> ExtractedPayload:
        ...
        models_to_try = [self.model_id] + [
            m for m in self.fallback_models if m != self.model_id
        ]
        last_error = None

        for m_id in models_to_try:
            try:
                response = client.invoke_model(modelId=m_id, body=body)
                ...
                last_error = e
                logger.warning(
                    f"Bedrock model '{m_id}' failed: {e}. Trying next model in chain."
                )
                continue

        raise RuntimeError(
            f"All Bedrock models in cascade failed. Last error: {last_error}"
        )
