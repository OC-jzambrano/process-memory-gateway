...
def test_bedrock_extract_three_benchmark_rules(memory_tools):
    ...
    assert len(result.candidates) >= 3, (
        f"Expected at least 3 candidates, got {len(result.candidates)}"
    )

    # Check rule types extracted
    rule_types = [c.rule_type for c in result.candidates]
    assert RuleType.APPROVAL_POLICY in rule_types or any(
        "approval" in c.rule_text.lower() for c in result.candidates
    )
...
def test_complete_lifecycle(memory_tools):
    ...
    memory_tools.review_candidate_rule(
        result.candidates[0].candidate_id, "approve", "reviewer"
    )
    # Reject second
    memory_tools.review_candidate_rule(
        result.candidates[1].candidate_id, "reject", "reviewer"
    )
