...
from src.models.schemas import CandidateRule, ExtractionSession, Principal
...
def test_client_a_rules_invisible_to_client_b(repo_two_clients):
    ...
    repo.review_candidate(
        "cand_a", DecisionType.APPROVE, "reviewer", client_id="client_a"
    )
...
def test_cannot_review_candidate_of_other_tenant(repo_two_clients):
    ...
    repo.create_session(
        ExtractionSession(
            session_id="sess_a3",
            client_id="client_a",
            interaction_text="Secret policy A",
            model_id="test",
        )
    )
    cand = CandidateRule(
        ...
        session_id="sess_a3",
        client_id="client_a",
        rule_text="Secret rule A",
        rule_type=RuleType.OPERATIONAL_CONSTRAINT,
        source_quote="Secret policy A",
        confidence=0.9,
    )
    repo.save_candidates([cand])
    ...
    with pytest.raises(ValueError, match="not found for tenant"):
        repo.review_candidate(
            "cand_a3", DecisionType.APPROVE, "bad_actor", client_id="client_b"
        )
...
def test_tools_principal_cross_tenant_forbidden(repo_two_clients):
    ...
    with pytest.raises(PermissionError, match="Cross-tenant access forbidden"):
        tools.extract_memory_candidates(
            "Dialogue", client_id="client_a", principal=principal_b
        )
