...
from src.models.enums import EnforcementMode, RuleType, Severity
...
def test_detects_approval_policy():
    ...
    assert any(
        r.enforcement_mode == EnforcementMode.REQUIRES_APPROVAL for r in result.rules
    )
...
def test_detects_operational_constraint():
    result = _parse(
        "Users must only execute migration during weekend maintenance windows."
    )
...
def test_detects_spanish_approval_policy():
    result = _parse(
        "Para instalar Fabricación se requiere la aprobación del líder de operaciones."
    )
    assert any(r.rule_type == RuleType.APPROVAL_POLICY for r in result.rules)
