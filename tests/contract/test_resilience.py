...
from src.models.enums import ExtractionMode, LLMProviderType, RuleType
...
def test_cascade_falls_back_to_local_when_all_fail():
    ...
    svc = ProcessMemoryExtractorService(
        provider=LLMProviderType.AUTO, openai_api_key="sk-mock-key"
    )
    ...
    with (
        ...
        patch.object(
            svc.bedrock_provider, "extract", side_effect=Exception("Bedrock throttled")
        ),
    ):
        result = svc._invoke_cascade(
            "Prompt", "Manufacturing requires approval from the lead."
        )
...
def test_openai_success_stops_cascade():
    ...
    svc = ProcessMemoryExtractorService(
        provider=LLMProviderType.OPENAI, openai_api_key="sk-test-mock-key"
    )
    ...
    with (
        patch.object(
            svc.openai_provider, "extract", return_value=mock_payload
        ) as mock_openai,
        patch.object(svc.bedrock_provider, "extract") as mock_bedrock,
    ):
...
def test_bedrock_fallback_when_openai_fails():
    ...
    with (
        patch.object(
            svc.openai_provider, "extract", side_effect=Exception("OpenAI rate limit")
        ),
        patch.object(
            svc.bedrock_provider, "extract", return_value=mock_bedrock_payload
        ) as mock_bedrock,
    ):
