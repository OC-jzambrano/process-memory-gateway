...
def test_review_events_are_strictly_immutable(repo, make_candidate, temp_db):
    ...
    repo.review_candidate(
        "cand_audit_01", DecisionType.APPROVE, "lead_reviewer", client_id="test_client"
    )
...
def test_cannot_replay_candidate_approval(repo, make_candidate):
    ...
    r1 = repo.review_candidate(
        "cand_replay_01", DecisionType.APPROVE, "reviewer", client_id="test_client"
    )
...
def test_cannot_supersede_already_superseded_rule(repo, make_candidate):
    ...
    v1 = repo.review_candidate(
        "cand_branch_01", DecisionType.APPROVE, "reviewer", client_id="test_client"
    )
    ...
    with pytest.raises(ValueError, match="cannot be superseded"):
        repo.supersede_rule(
            v1.rule_id, "divergent v2 rule", "reviewer", client_id="test_client"
        )
