...
def rsa_keypair():
    ...
    def to_b64(val: int, length: int = 256) -> str:
        import base64
        return base64.urlsafe_b64encode(val.to_bytes(length, byteorder="big")).decode("utf-8").rstrip("=")
...
def auth_setup(rsa_keypair, tmp_path_factory):
    ...
    # Company A (Active)
    test_repo.upsert_company(Company(
    ...
    test_repo.upsert_membership(Membership(
...
def test_missing_required_scope_returns_401(auth_setup):
    client = auth_setup["client"]
    token = make_token(auth_setup["private_key"], scope="openid email") # Missing mcp:tools
