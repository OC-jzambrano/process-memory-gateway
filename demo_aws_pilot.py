...
def run_pilot_demo():
    ...
    print(
        f"  Target Scope: {stage_res.scope.system}:{stage_res.scope.application}:{stage_res.scope.resource}:{stage_res.scope.operation}"
    )
    print(
        f"  Proposed Constraint: {stage_res.constraint.kind.value} on field '{stage_res.constraint.field}'"
    )
    ...
    print(
        f"  Active Canonical Rules: {len(active)} (Pending candidates have ZERO operational effect)"
    )
    ...
    print(
        f"  Review Decision: {review_res.decision.value} -> Rule #{review_res.rule_id} (v{review_res.version})"
    )
    ...
    print(
        f"  Odoo Task ID: {retry_res.odoo_task_id} (Returned cached result without creating duplicate task)"
    )
    ...
    print(
        f"  Company 2 Task Status: {co2_res.status.value} (DoD is NOT required for Company 2)"
    )
