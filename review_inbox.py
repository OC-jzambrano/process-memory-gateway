...
def interactive_review_session():
    ...
    reviewer_name = (
        input("Enter your name (Reviewer ID) [default: juan_zambrano]: ").strip()
        or "juan_zambrano"
    )
    client_id = (
        input("Enter Client ID [default: odooconcept_demo]: ").strip()
        or "odooconcept_demo"
    )

    principal = Principal(
        client_id=client_id, user_id=reviewer_name, role="human_reviewer"
    )

    pending_candidates = tools.get_candidate_rules(
        client_id=client_id, status=RuleStatus.PENDING_REVIEW, principal=principal
    )
    ...
    for idx, cand in enumerate(pending_candidates, 1):
        ...
        print(
            f"  • Severity:         {cand.severity.value} (Mode: {cand.enforcement_mode.value})"
        )
        ...
        choice = (
            input("Action: [A]pprove | [R]eject | [E]dit & Approve | [S]kip : ")
            .strip()
            .upper()
        )
        ...
        if choice == "A":
            ...
            print(
                f"  [SUCCESS] Approved! Created Canonical Rule '{rule.rule_id}' (version {rule.version}).\n"
            )
            ...
            print(
                f"  [SUCCESS] Rejected candidate '{cand.candidate_id}'. Marked as rejected.\n"
            )
            ...
            print(
                f"  [SUCCESS] Edited & Approved! Created Canonical Rule '{rule.rule_id}' with your refined text.\n"
            )
